...
(defn linkify
  ...
  (let [long-form (re-matches #"(.+)\s*\|\s*(.+)" (second match))
        ...
        url (pages/route-for :view_document (assoc (-> request :params) :slug slug))]
...
(defn index
  [request]
  (let [pager (admin-helpers/add-pagination (model/gather :document {:order {:created_at :asc}})
                                            {:page-size 25
  ...
  (let [slug-or-id (-> request :params :slug)
    ...
    (if (not (nil? document))
      ...
      (if (-> request :session :admin :user)
        (redirect (pages/route-for :new_document {:slug slug-or-id}))
...
(defn save
  ...
  (let [params (-> request :params)
        ...
        authored (assoc document :author_id (-> request :session :admin :user :id))
        saved (model/create :document authored)
        ;;deparented (assoc authored :id nil :parent_id nil)
        ;;saved (model/create :document deparented)
        ;;obviated (when-not (nil? picked)
        ;;           (model/create :document (assoc picked :parent_id (:id saved))))
        ]
    (redirect (pages/route-for :view_document {:slug (:slug saved)}))))
